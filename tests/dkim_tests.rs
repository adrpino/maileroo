...
async fn test_dkim_rotation_e2e_flow() {
    common::run_on_all_dbs(|db| async move {
        ...
        let auth_cookie =
            common::get_auth_cookie(app_router.clone(), &user_email, "super-secure-password").await;
        ...
        let test_domain = domains
            .iter()
            .find(|d| d.name == "example-rotation-test.com")
            .expect("Domain should have been inserted into database");

        assert!(
            test_domain.dkim_private_key.is_some(),
            "Active DKIM private key should be generated"
        );
        assert!(
            test_domain.dkim_public_key.is_some(),
            "Active DKIM public key should be generated"
        );
        ...
        let body_bytes = axum::body::to_bytes(response_rotate.into_body(), usize::MAX)
            .await
            .unwrap();
        let body_string = String::from_utf8_lossy(&body_bytes);
        assert!(
            body_string.contains("Pending DKIM Key Verification"),
            "Should render pending verification section"
        );
        assert!(
            body_string.contains("maileroo2._domainkey"),
            "Should render new selector host"
        );

        // Verify the database state has the pending key set
        let test_domain_rotated = maileroo::db::get_domain_by_id(&db, test_domain.id)
            .await
            .unwrap()
            .unwrap();
        ...
        assert_eq!(
            test_domain_rotated.pending_dkim_selector,
            Some("maileroo2".to_string())
        );
        ...
        let body_bytes_verify = axum::body::to_bytes(response_verify.into_body(), usize::MAX)
            .await
            .unwrap();
        let body_string_verify = String::from_utf8_lossy(&body_bytes_verify);
        assert!(
            body_string_verify.contains("DNS Verification failed"),
            "Should render failure message inside the template"
        );
        ...
        let body_bytes_cancel = axum::body::to_bytes(response_cancel.into_body(), usize::MAX)
            .await
            .unwrap();
        let body_string_cancel = String::from_utf8_lossy(&body_bytes_cancel);
        assert!(
            !body_string_cancel.contains("Pending DKIM Key Verification"),
            "Pending section should be gone after cancellation"
        );

        // Verify the database state cleared the pending columns
        let test_domain_cancelled = maileroo::db::get_domain_by_id(&db, test_domain.id)
            .await
            .unwrap()
            .unwrap();
