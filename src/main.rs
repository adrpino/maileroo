...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
            ...
            match db::delete_old_emails(&cleanup_pool, email_retention_days).await {
                Ok(body_keys) => {
                    for key in &body_keys {
                        ...
                        {
                            tracing::error!("Failed to delete email file {:?}: {}", file_path, e);
                        }
                    }
    ...
    let smtp_server = inbound::run_server(
        &smtp_addr,
        db_pool,
        storage_dir.clone(),
        outbound,
    ...
    // Web server is always HTTP (behind Caddy in prod, or local dev)
    let web_server = web::run_web_server(&web_addr, web_state, None);
